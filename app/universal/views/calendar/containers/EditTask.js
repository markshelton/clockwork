...
import { actions } from "../../../state/redux/ducks/calendar";
...
class EditTask extends Component {
  ...
  onDeleteClick = () => {
    this.props.delete_task(this.props.match.params.id);
  };
  onFormSubmit = task => {
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    errorMessage: state.calendar.error
  };
};
...
const loadData = (store, req) => {
  const { id } = req.params;
  const action = actions.fetch_task(id);
